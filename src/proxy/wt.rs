...
pub async fn serve_session(
    ...
    // When any loop exits (error or client close), cancel the others so
    // the session ends promptly and wt_active_sessions decrements exactly once.
    tokio::select! {
        _ = bidi_echo_loop(session.clone(), remote_addr, Arc::clone(&metrics), cancel.child_token()) => {},
        _ = uni_echo_loop(session.clone(), remote_addr, Arc::clone(&metrics), cancel.child_token()) => {},
        _ = datagram_echo_loop(session.clone(), remote_addr, Arc::clone(&metrics), cancel.child_token()) => {},
    }
...
async fn bidi_echo_loop(
    ...
    loop {
        tokio::select! {
            _ = cancel.cancelled() => {
                debug!("WT bidi loop cancelled (remote={})", remote_addr);
                return;
            }
...
async fn uni_echo_loop(
    ...
    loop {
        tokio::select! {
            _ = cancel.cancelled() => {
                debug!("WT uni loop cancelled (remote={})", remote_addr);
                return;
            }
            result = session.accept_uni() => match result {
...
async fn datagram_echo_loop(
    ...
    loop {
        tokio::select! {
            _ = cancel.cancelled() => {
                debug!("WT datagram loop cancelled (remote={})", remote_addr);
                return;
            }
...
mod tests {
    ...
    /// When one WT echo loop exits, the CancellationToken must cause the
    /// others to exit promptly so wt_active_sessions decrements exactly once.
    #[tokio::test]
    async fn test_wt_session_cancels_remaining_loops() {
        let cancel = tokio_util::sync::CancellationToken::new();
        ...
        // Simulate one loop exiting early
