...
impl UpstreamManager {
    ...
    pub fn new(
        ...
        for (name, pool_config) in &config.upstreams {
            ...
            for d in discovered {
                pool.add_backend(BackendConfig::from(d));
            }
    ...
    /// Returns an arbitrary pool (useful when no specific pool is requested).
    pub fn first_pool(&self) -> Option<Arc<UpstreamPool>> {
        self.pools.iter().next().map(|p| Arc::clone(&p))
    }
