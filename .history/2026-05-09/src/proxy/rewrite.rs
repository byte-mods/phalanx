...
fn nginx_to_regex_replacement(replacement: &str) -> String {
    ...
    // $$ becomes a literal $ (nginx escaping rule).
    ...
    while i < chars.len() {
        if chars[i] == '$' && i + 1 < chars.len() {
            // $$ → literal $ (passed through as $$ so regex::replacen interprets it correctly)
            if chars[i + 1] == '$' {
                out.push_str("$$");
                i += 2;
                continue;
...
mod tests {
    ...
    #[test]
    fn test_nginx_to_regex_replacement_double_dollar_escapes_literal() {
        // $$ stays $$ in the intermediate so regex::replacen emits a literal $
        assert_eq!(nginx_to_regex_replacement("/price/$$100"), "/price/$$100");
    }

    #[test]
    fn test_nginx_to_regex_replacement_double_dollar_with_capture() {
        // $$1 → literal $ followed by text 1 (not a capture group)
        assert_eq!(nginx_to_regex_replacement("/$$1/item"), "/$$1/item");
    }

    #[test]
    fn test_rewrite_double_dollar_emits_literal_dollar_end_to_end() {
        let rule = RewriteRule::compile(r"^/price/(.+)$", "/cost/$$1", "break").unwrap();
        let result = rule.apply("/price/99");
        assert_eq!(result, Some("/cost/$1".to_string()));
    }

    #[test]
    fn test_rewrite_double_dollar_with_capture_group_end_to_end() {
        // User wants: capture group 1 followed by literal $
        let rule = RewriteRule::compile(r"^/old/(.+)$", "/new/$1$$", "break").unwrap();
        let result = rule.apply("/old/path");
